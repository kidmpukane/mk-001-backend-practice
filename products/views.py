...
def get_all_products(request):
    ...
    secondary_serializer = SecondaryProductSerializer(
        secondary_products, many=True)
...
def get_secondary_product(request, secondary_id):
    try:
        secondary_product_data, similar_secondary_products = get_product_and_similars(
            secondary_id, SecondaryProduct)
...
def get_all_products_features():
    primary_feature_lists = list(
        PrimaryProduct.objects.values_list('feature_list', flat=True))
    secondary_feature_lists = list(
        SecondaryProduct.objects.values_list('feature_list', flat=True))
    all_feature_lists = list(
        chain(primary_feature_lists, secondary_feature_lists))
    ...
    if serializer.is_valid():
        ...
        for idx in similar_indices:
            product = get_object_or_404(
                chain(PrimaryProduct.objects.all(), SecondaryProduct.objects.all()), pk=idx)
            product_data = {
                "id": product.id,
